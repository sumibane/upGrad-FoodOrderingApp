...
class Checkout extends Component {
    ...
    // Get All the saved address of Customer
    getAllAddress = () => {
        let data = null;
        let that = this;
    ...
    //Update the Columns as per screen size
    getGridListColumn = () => {
    ...
    //Apply the cupon code if appropriate
    ...
    //Check the customer Login status, redirect if not
    redirectToHome = () => {
        if (!this.state.isLoggedIn) {
            return <Redirect to="/" />
        }
    ...
    render() {
        ...
        return (
            <div>
                ...
                {this.redirectToHome()}
                {/*Import Header and remove teh Search Box*/}
